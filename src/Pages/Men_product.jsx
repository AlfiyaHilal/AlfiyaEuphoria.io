...
const MenProduct = () => {
  ...
  const [activeImage, setActiveImage] = useState(0); // To keep track of the active thumbnail
  ...
  // Default size and color options if product data is not available yet
  const sizesToShow = product?.size || ["S", "M", "L", "XL"];
  const colorsToShow = product?.colors || ["Red", "Blue", "Black", "White"];
  ...
  // Fetch similar products details from API
    ...
    <>
          ...
          <ImageSlider>
            {imagesToShow.map((image, index) => (
              <Thumbnail
                ...
                onClick={() => setActiveImage(index)} // Change active image on thumbnail click
                alt="Product thumbnail"
              />
            ))}
            <ArrowButton onClick={() => setActiveImage((activeImage + 1) % imagesToShow.length)}>
              &#8595; 
            </ArrowButton>
          </ImageSlider>
      ...
      <RightSection>
        ...
        <RatingSection>
          ...
          <Ratings>{product.rating.rate}/5</Ratings>
        ...
        <ProductGrid>
          {similarProducts.map((product) => (
            <ProductCard key={product.id}>
              ...
              <SimilarProductImage src={`${product.image}`} alt="No-Image" onClick={() => handleProductClick(product.id)} />
              <CardDetails>
                <LeftSide>
                  <ProductName>{product.title}</ProductName>
                  <BrandName>Otto</BrandName> {/* Displaying the brand name */}
                </LeftSide>
                <RightSide>
                  <PriceBox>
                    <ProductPrice>${product.rating.count}</ProductPrice> {/* Displaying price */}
                  </PriceBox>
                </RightSide>
              </CardDetails>
            </ProductCard>
